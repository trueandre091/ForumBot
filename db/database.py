...
async def get_contacts_by_activity_area(activity_areas):
    async with aiosqlite.connect(DB_NAME) as db:
        query = '''
            SELECT * FROM contacts 
            WHERE ''' + ' OR '.join([f"activity_area LIKE ?" for _ in activity_areas])
        values = [f"%{area}%" for area in activity_areas]
...
async def update_meeting(meeting_id: int, **kwargs):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        try:
            ...
            await db.commit()
            print(f"Встреча с id {meeting_id} обновлена. Измененные параметры: {kwargs}")
...
async def get_meetings_with_status_zero() -> list[dict]:
    """Возвращает список словарей встреч, где status = 0."""
    async with aiosqlite.connect(DB_NAME) as conn:
        cursor = await conn.execute("SELECT * FROM meetings WHERE status = 0")
        results = await cursor.fetchall()
        columns = [column[0] for column in cursor.description]
